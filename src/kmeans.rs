...
impl Kmeans {
    ...
    pub fn finalize(self, palette: &mut PalF) -> f64 {
        for (avg, (color, pop)) in self
            .averages
            .iter()
            .zip(palette.iter_mut())
            .filter(|(_, (_, pop))| !pop.is_fixed())
        {
            let total = avg.total;
            ...
            .map(|item| {
                ...
                if adjust_weight {
                    ...
                    item.adjusted_weight =
                        2.0f32.mul_add(item.adjusted_weight, item.perceptual_weight) * (0.5 + diff);
                }
...
fn replace_unused_colors(palette: &mut PalF, hist: &HistogramInternal) -> Result<(), Error> {
    for pal_idx in 0..palette.len() {
        let Some(pop) = palette.pop_as_slice().get(pal_idx) else {
            break;
        };
        if pop.popularity() == 0. && !pop.is_fixed() {
            ...
            for item in hist.items.iter() {
                ...
                let may_be_worst = colors
                    .get(item.likely_palette_index() as usize)
