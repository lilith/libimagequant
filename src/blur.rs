/// Scalar implementation - LLVM auto-vectorizes this very effectively (~11 Gelem/s)
#[doc(hidden)]
pub fn liq_max3_scalar_ref(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    for j in 0..height {
        let row = &src[j * width..][..width];
        let dst_row = &mut dst[j * width..][..width];
        let prevrow = &src[j.saturating_sub(1) * width..][..width];
        let nextrow = &src[(j + 1).min(height - 1) * width..][..width];
        let mut prev: u8;
        let mut curr = row[0];
        let mut next = row[0];
        for i in 0..width - 1 {
            prev = curr;
            curr = next;
            next = row[i + 1];
            let t1 = prev.max(next);
            let t2 = nextrow[i].max(prevrow[i]);
            dst_row[i] = curr.max(t1).max(t2);
        }
        let t1 = curr.max(next);
        let t2 = nextrow[width - 1].max(prevrow[width - 1]);
        dst_row[width - 1] = curr.max(t1).max(t2);
    }
}

/// Scalar implementation - LLVM auto-vectorizes this very effectively (~11 Gelem/s)
#[doc(hidden)]
pub fn liq_min3_scalar_ref(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    for j in 0..height {
        let row = &src[j * width..][..width];
        let dst_row = &mut dst[j * width..][..width];
        let prevrow = &src[j.saturating_sub(1) * width..][..width];
        let nextrow = &src[(j + 1).min(height - 1) * width..][..width];
        let mut prev: u8;
        let mut curr = row[0];
        let mut next = row[0];
        for i in 0..width - 1 {
            prev = curr;
            curr = next;
            next = row[i + 1];
            let t1 = prev.min(next);
            let t2 = nextrow[i].min(prevrow[i]);
            dst_row[i] = curr.min(t1).min(t2);
        }
        let t1 = curr.min(next);
        let t2 = nextrow[width - 1].min(prevrow[width - 1]);
        dst_row[width - 1] = curr.min(t1).min(t2);
    }
}

/// Blurs image horizontally (width 2*size+1) and writes it transposed to dst (called twice gives 2d blur)
#[inline(never)]
fn transposing_1d_blur(src: &[u8], dst: &mut [u8], width: usize, height: usize, size: u16) {
    if width < 2 * size as usize + 1 || height < 2 * size as usize + 1 {
        return;
    }

    for (j, row) in src.chunks_exact(width).enumerate() {
        let mut sum = u16::from(row[0]) * size;
        for &v in &row[0..size as usize] {
            sum += u16::from(v);
        }
        for i in 0..size as usize {
            sum -= u16::from(row[0]);
            sum += u16::from(row[i + size as usize]);
            dst[i * height + j] = (sum / (size * 2)) as u8;
        }
        for i in size as usize..width - size as usize {
            sum -= u16::from(row[i - size as usize]);
            sum += u16::from(row[i + size as usize]);
            dst[i * height + j] = (sum / (size * 2)) as u8;
        }
        for i in width - size as usize..width {
            sum -= u16::from(row[i - size as usize]);
            sum += u16::from(row[width - 1]);
            dst[i * height + j] = (sum / (size * 2)) as u8;
        }
    }
}

/// Picks maximum of neighboring pixels (blur + lighten)
#[inline(never)]
#[doc(hidden)]
pub fn liq_max3(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    liq_max3_scalar_ref(src, dst, width, height);
}

/// Picks minimum of neighboring pixels (blur + darken)
#[inline(never)]
#[doc(hidden)]
pub fn liq_min3(src: &[u8], dst: &mut [u8], width: usize, height: usize) {
    liq_min3_scalar_ref(src, dst, width, height);
}

/// Filters src image and saves it to dst, overwriting tmp in the process.
/// Image must be width*height pixels high. Size controls radius of box blur.
pub(crate) fn liq_blur(src_dst: &mut [u8], tmp: &mut [u8], width: usize, height: usize, size: u16) {
    transposing_1d_blur(src_dst, tmp, width, height, size);
    transposing_1d_blur(tmp, src_dst, height, width, size);
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_max3_consistency() {
        for width in [17, 32, 64, 100, 128, 255] {
            for height in [3, 10, 32] {
                let src: Vec<u8> = (0..width * height)
                    .map(|i| ((i * 17 + i / 3) % 256) as u8)
                    .collect();
                let mut dst1 = vec![0u8; width * height];
                let mut dst2 = vec![0u8; width * height];

                liq_max3(&src, &mut dst1, width, height);
                liq_max3_scalar_ref(&src, &mut dst2, width, height);

                assert_eq!(dst1, dst2, "max3 mismatch at width={width}, height={height}");
            }
        }
    }

    #[test]
    fn test_min3_consistency() {
        for width in [17, 32, 64, 100, 128, 255] {
            for height in [3, 10, 32] {
                let src: Vec<u8> = (0..width * height)
                    .map(|i| ((i * 17 + i / 3) % 256) as u8)
                    .collect();
                let mut dst1 = vec![0u8; width * height];
                let mut dst2 = vec![0u8; width * height];

                liq_min3(&src, &mut dst1, width, height);
                liq_min3_scalar_ref(&src, &mut dst2, width, height);

                assert_eq!(dst1, dst2, "min3 mismatch at width={width}, height={height}");
            }
        }
    }
}
