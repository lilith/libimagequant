...
pub type liq_log_callback_function =
    unsafe extern "C" fn(liq: &liq_attr, message: *const c_char, user_info: AnySyncSendPtr);
pub type liq_log_flush_callback_function =
...
macro_rules! bad_object {
    ($obj:expr, $tag:expr) => {{
        ...
        let bork = if cfg!(miri) {
            false
        } else {
            unsafe { liq_received_invalid_pointer((obj as *const _ as *const u8)) }
        };
...
impl Drop for liq_attr {
    fn drop(&mut self) {
        if bad_object!(self, LIQ_ATTR_MAGIC) {
            return;
        }
...
impl Drop for liq_image<'_> {
    fn drop(&mut self) {
        if bad_object!(self, LIQ_IMAGE_MAGIC) {
            return;
        }
        unsafe {
...
impl Drop for liq_result {
    fn drop(&mut self) {
        if bad_object!(self, LIQ_RESULT_MAGIC) {
            return;
        }
        self.magic_header = LIQ_FREED_MAGIC;
    }
...
impl Drop for liq_histogram {
    fn drop(&mut self) {
        if bad_object!(self, LIQ_HISTOGRAM_MAGIC) {
            return;
        }
        self.magic_header = LIQ_FREED_MAGIC;
    }
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_attr_set_progress_callback(
    attr: &mut liq_attr,
    callback: liq_progress_callback_function,
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_result_set_progress_callback(
    ...
    result.inner.set_progress_callback(move |f| {
        if callback(f, user_info) == 0 {
            ControlFlow::Break
        } else {
            ControlFlow::Continue
...
unsafe fn attr_to_liq_attr_ptr(ptr: &Attributes) -> &liq_attr {
    ...
    &*(ptr as *const Attributes)
        .cast::<u8>()
        .offset(outer_addr - inner_addr)
        .cast::<liq_attr>()
}

#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_set_log_callback(
    attr: &mut liq_attr,
    callback: liq_log_callback_function,
    user_info: AnySyncSendPtr,
) {
    if bad_object!(attr, LIQ_ATTR_MAGIC) {
        return;
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_set_log_flush_callback(
    attr: &mut liq_attr,
    callback: liq_log_flush_callback_function,
    user_info: AnySyncSendPtr,
) {
    if bad_object!(attr, LIQ_ATTR_MAGIC) {
        return;
    ...
    attr.inner
        .set_log_flush_callback(move |attr| callback(attr_to_liq_attr_ptr(attr), user_info));
}
...
pub extern "C" fn liq_set_min_posterization(attr: &mut liq_attr, bits: c_int) -> liq_error {
    ...
    attr.inner
        .set_min_posterization(bits as u8)
        .err()
        .unwrap_or(LIQ_OK)
}
...
pub extern "C" fn liq_set_quality(
    ...
    attr.inner
        .set_quality(minimum as u8, target as u8)
        .err()
        .unwrap_or(LIQ_OK)
}
...
pub extern "C" fn liq_quantize_image(
    ...
    attr.quantize(img).ok().map(|inner| {
        Box::new(liq_result {
...
pub unsafe extern "C" fn liq_write_remapped_image(
    ...
    if liq_received_invalid_pointer(buffer_bytes.cast()) {
        return Error::InvalidPointer;
    }
    ...
    liq_write_remapped_image_impl(result, input_image, buffer_bytes)
        .err()
        .unwrap_or(LIQ_OK)
}
...
pub unsafe extern "C" fn liq_write_remapped_image_rows(
    ...
    if liq_received_invalid_pointer(row_pointers.cast()) {
        return Error::InvalidPointer;
    }
    let rows = slice::from_raw_parts_mut(row_pointers, input_image.height());

    liq_write_remapped_image_rows_impl(result, input_image, rows)
        .err()
        .unwrap_or(LIQ_OK)
}
...
pub extern "C" fn liq_histogram_add_fixed_color(
    ...
    if bad_object!(hist, LIQ_HISTOGRAM_MAGIC) {
        return Error::InvalidPointer;
    }
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn liq_image_set_background<'pixels>(
    img: &mut liq_image<'pixels>,
    background: Box<liq_image<'pixels>>,
) -> liq_error {
    if bad_object!(img, LIQ_IMAGE_MAGIC) || bad_object!(background, LIQ_IMAGE_MAGIC) {
        return Error::InvalidPointer;
    }
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_image_set_importance_map(
    img: &mut liq_image,
    importance_map: *mut u8,
    buffer_size: usize,
    ownership: liq_ownership,
) -> liq_error {
    if bad_object!(img, LIQ_IMAGE_MAGIC) {
        return Error::InvalidPointer;
    }
    ...
    if buffer_size == 0 || liq_received_invalid_pointer(importance_map) {
        return Error::InvalidPointer;
    }
    ...
    if ownership == liq_ownership::LIQ_COPY_PIXELS {
        img.set_importance_map(importance_map_slice)
            .err()
            .unwrap_or(LIQ_OK)
...
pub unsafe extern "C" fn liq_image_set_memory_ownership(
    ...
    liq_image_set_memory_ownership_impl(&mut img.inner, own_rows, own_pixels, img.c_api_free)
        .err()
        .unwrap_or(LIQ_OK)
}
...
pub extern "C" fn liq_attr_create_with_allocator(
    ...
    debug_assert_eq!(
        ptr::addr_of!(*attr),
        unsafe { attr_to_liq_attr_ptr(&attr.inner) } as *const liq_attr
    );
    Some(attr)
}
...
#[cfg_attr(
    ...
    deprecated(
        note = "Cargo features configuration issue: both std and no_std features are enabled in imagequant-sys\nYou must disable default features to use no_std."
    )
)]
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn liq_image_quantize(
    img: &mut liq_image,
    attr: &mut liq_attr,
    ...
    let res = attr.quantize(img).map(|inner| liq_result {
        magic_header: LIQ_RESULT_MAGIC,
        inner,
    });
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn liq_histogram_quantize(
    hist: &mut liq_histogram,
    attr: &liq_attr,
    write_only_output: &mut MaybeUninit<Option<Box<liq_result>>>,
) -> liq_error {
    ...
    let res = hist.quantize(attr).map(|inner| liq_result {
        magic_header: LIQ_RESULT_MAGIC,
        inner,
    });
    store_boxed_result(res, write_only_output)
...
#[inline]
fn store_boxed_result<T>(
    res: Result<T, liq_error>,
    out: &mut MaybeUninit<Option<Box<T>>>,
) -> liq_error {
    match res {
        Ok(res) => {
            out.write(Some(Box::new(res)));
            LIQ_OK
        }
        Err(err) => {
            out.write(None);
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_image_create_custom(
    attr: &liq_attr,
    row_callback: liq_image_get_rgba_row_callback,
    user_info: AnySyncSendPtr,
    width: c_uint,
    height: c_uint,
    gamma: f64,
...
pub unsafe extern "C" fn liq_image_create_rgba_rows<'rows>(
    ...
    liq_image_create_rgba_rows_impl(&attr.inner, rows, width as _, height as _, gamma).map(
        move |inner| {
            Box::new(liq_image {
                magic_header: LIQ_IMAGE_MAGIC,
                inner: ManuallyDrop::new(inner),
...
#[no_mangle]
#[inline(never)]
pub unsafe extern "C" fn liq_image_create_rgba<'pixels>(
    attr: &liq_attr,
    pixels: *const liq_color,
    width: c_uint,
    height: c_uint,
    gamma: f64,
) -> Option<Box<liq_image<'pixels>>> {
    if liq_received_invalid_pointer(pixels.cast()) {
        return None;
    }
    if !check_image_size(attr, width, height) {
        return None;
    ...
    let rows = (0..height as usize)
        .map(move |i| pixels.add(width as usize * i))
        .collect();
    liq_image_create_rgba_bitmap_impl(&attr.inner, rows, width as _, height as _, gamma).map(
        move |inner| {
            Box::new(liq_image {
                magic_header: LIQ_IMAGE_MAGIC,
                inner: ManuallyDrop::new(inner),
...
pub unsafe extern "C" fn liq_histogram_add_colors(
    ...
    if bad_object!(attr, LIQ_ATTR_MAGIC) || bad_object!(input_hist, LIQ_HISTOGRAM_MAGIC) {
        return Error::InvalidPointer;
    }
    ...
    if liq_received_invalid_pointer(entries.cast()) {
        return Error::InvalidPointer;
    }
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn liq_histogram_add_image(
    input_hist: &mut liq_histogram,
    attr: &liq_attr,
    input_image: &mut liq_image,
) -> liq_error {
    if bad_object!(attr, LIQ_ATTR_MAGIC)
        ...
        || bad_object!(input_image, LIQ_IMAGE_MAGIC)
    {
        return Error::InvalidPointer;
    }
    let attr = &attr.inner;
...
fn c_callback_test_c() {
    ...
    let mut res = unsafe {
        ...
        let mut img = liq_image_create_custom(
            &a,
            get_row,
            AnySyncSendPtr(ptr::addr_of_mut!(called).cast::<c_void>()),
            123,
            5,
            0.,
        )
...
fn ownership_bitflags() {
    assert_eq!(
        4 + 16,
        (liq_ownership::LIQ_OWN_ROWS | liq_ownership::LIQ_COPY_PIXELS).bits()
    );
}

#[cfg(all(
    feature = "no_std_global_handlers",
    not(feature = "std"),
    feature = "no_std"
))]
#[cfg(not(test))]
